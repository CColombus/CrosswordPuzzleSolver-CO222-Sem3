...
#define MAX_SIZE 50 //define the max width of a puzzle
...
/*
    d_keep_0 = formatted grid to obtain "conwho" details of horizontal blocks
    d_keep_1 = formatted grid to obtain "conwho" details of vertical blocks
    d_keep_2 = formatted grid to obtain "conid" details of horizontal blocks
    d_keep_3 = formatted grid to obtain "conid" details of vertical blocks
    d_keep_4 = exact copy of the original grid for printing answers
    d_keep_5 = formatted grid for finding single width spaces (edge case)
*/

char *d_keep_0,  *d_keep_4;
//char *d_keep_1, *d_keep_2, *d_keep_3;
...
int spaceHrz(char grid[][MAX_SIZE], int rows, int cols, int n); // funtion to get the horizontal block information
int spaceVrt(char grid[][MAX_SIZE], int rows, int cols, int m); // function to get the vertical block information
...
int gridRead(char grid[][MAX_SIZE], int rows, int cols);        // function which initiate the grid reading process
...
int main()
{
    char grid[MAX_SIZE][MAX_SIZE]; // to store the given ccrossword puzzle grid
    int rows, cols;

    // get each row of puzzle grid one by one
    for (rows = 0; fgets(grid[rows], MAX_SIZE, stdin)[0] != '\n'; rows++)
    {
    ...
    for (word_count = 0; fgets(wordlist[word_count], 30, stdin)[0] != '\n'; word_count++)
    { // input validation
...
int gridRead(char grid[][MAX_SIZE], int rows, int cols) // function which initiate the grid reading process
{
    ...
    space = (sspace **)malloc(word_count * sizeof(sspace *));
...
int spaceHrz(char grid[][MAX_SIZE], int rows, int cols, int n)
{
    char *keep_0[rows];
    for (int i = 0; i < rows; i++)
    {
...
int spaceVrt(char grid[][MAX_SIZE], int rows, int cols, int m)
{
